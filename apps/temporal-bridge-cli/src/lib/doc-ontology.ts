...
import { EdgeTypeSchema, EntityTypeSchema, entityFields } from '@getzep/zep-cloud';
...
export const ArchitectureSchema: EntityType = {
  description:
    "Architectural component, system design element, or infrastructure piece that forms part of the system's structure",
  fields: {
    component_type: entityFields.text(
...
export const DataModelSchema: EntityType = {
  description:
    'Data structure, schema definition, or information model that defines how data is organized and accessed',
  fields: {
...
export const ArchitectureDecisionSchema: EntityType = {
  description:
    'Architecture Decision Record (ADR) documenting a significant architectural choice, its context, and consequences',
  fields: {
...
export function validateOntologyLimits(): {
  ...
  if (ONTOLOGY_LIMITS.CURRENT_ENTITY_COUNT > ONTOLOGY_LIMITS.MAX_ENTITY_TYPES) {
    errors.push(
      `Entity types (${ONTOLOGY_LIMITS.CURRENT_ENTITY_COUNT}) exceed limit (${ONTOLOGY_LIMITS.MAX_ENTITY_TYPES})`,
    );
  }
